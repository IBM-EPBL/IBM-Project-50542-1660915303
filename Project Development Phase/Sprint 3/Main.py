...
client = Cloudant.iam('f44077b9-11a9-4d47-96c3-ff580f6f02ac-bluemix','mE2nyYXhST6WYSebM3wJsr8Dsn30GdXePyM4hMx5YvLr', connect=True) 
name = 'name'
email = 'a@b.c'
password = '123'
...
def image_database_updation(name,email,imagestr): 
    ...
    json_image_document={ 'name':name, 'email':email, 'image':imagestr,'datetime':now.strftime("%m/%d/%Y, %H:%M:%S")}
...
def prediction():
    if(flask.request.method=='POST'): 
        ...
        '''img_retrived = np.asarray(base64.b64decode(data[flask_login.current_user.id]['image'])) print(data[flask_login.current_user.id]['image'])
print(img_retrived.shape)'''

        #img_retrived = np.resize(img_retrived,(244,244)) 
        img_retrive = Image.fromarray(img_retrived) 
        img_retrive.save('static/image data/sae.png') 

        '''img_retrived = np.frombuffer(BytesIO(base64.b64decode(data[flask_login.current_user.id]['image'])))'''
        ...
        if(result1 == 'front' and result2 == 'minor'):
            value = 'Cost=3000 - 5000 INR , Percentage=10% , Type=car front minor damage' 
        elif(result1 == 'front' and result2 == 'moderate'):
            value = 'Cost=6000 - 8000 INR , Percentage=20% , Type=car front moderate damage'
        elif(result1 == 'front' and result2 == 'severe'):
            value = 'Cost=9000 - 11000 INR , Percentage=30% , Type=car front severe damage'
        elif(result1 == 'rear' and result2 == 'minor'):
            value = 'Cost=4000 - 6000 INR , Percentage=40% , Type=car rear minor damage'
        elif(result1 == 'rear' and result2 == 'moderate'):
            value = 'Cost=7000 - 9000 INR , Percentage=50% , Type=car rear moderate damage' 
        elif(result1 == 'rear' and result2 == 'severe'):
            value = 'Cost=11000 - 13000 INR , Percentage=60% , Type=car rear severe damage' 
        elif(result1 == 'side' and result2 == 'minor'):
            value = 'Cost=6000 - 8000 INR , Percentage=70% , Type=car side minor damage'
        elif(result1 == 'side' and result2 == 'moderate'):
            value = 'Cost=9000- 11000 INR , Percentage=80% , Type=car side moderate damage' 
        elif(result1 == 'side' and result2 == 'severe'):
            value = 'Cost=12000 - 15000 INR , Percentage=90% , Type=car side severe damage'
        else:
            value = '16000 - 50000 INR , Percentage=100% ,Type=heavy damage' 
        ...
        return render_template('prediction.html',flash_message='True',value = result1+' '+result2+' '+value) #,imag=img_retrived)
